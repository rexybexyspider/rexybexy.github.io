...
function setupEventListeners() {
    ...
    // progress bar کلیک (درست شده برای RTL)
    // در حالت RTL: از سمت راست شروع می‌شه، پس باید محاسبه برعکس بشه
    function seekFromEvent(e, barElement) {
        const rect = barElement.getBoundingClientRect();
        const isRTL = document.documentElement.dir === 'rtl' || document.body.dir === 'rtl';
        let pct;
        if (isRTL) {
            // در RTL: clientX نزدیک به rect.right یعنی 0% (شروع)
            // clientX نزدیک به rect.left یعنی 100% (پایان)
            pct = (rect.right - e.clientX) / rect.width;
        } else {
            // در LTR: clientX نزدیک به rect.left یعنی 0% (شروع)
    ...
    // همچنین support برای drag (کشیدن) روی progress bar
    let isDragging = false;
    function setupDrag(barId) {
        const bar = $(barId);
        if (!bar) return;
        ...
        const moveDrag = (e) => {
            ...
            seekFromEvent(e.touches ? e.touches[0] : e, bar);
        };
        const endDrag = () => {
            isDragging = false;
        };
    ...
    setupDrag('progress-bar');
    setupDrag('np-progress-bar');
